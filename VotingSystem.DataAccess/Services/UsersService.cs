...
namespace VotingSystem.DataAccess.Services
{
    public class UsersService : IUsersService
    {
        ...
        public async Task AddUserAsync(User user, string password)
        {
            ...
            if (!result.Succeeded)
                throw new ApiException($"Sikertelen felhasználó létrehozás: {result.Errors.First().Description}");
        }
        ...
        public async Task<(string authToken, string refreshToken, string userId)> LoginAsync(string email, string password)
        {
            ...
            if (result.IsLockedOut)
                throw new AccessViolationException("Túl sok hibás bejelentkezés.");
            if (!result.Succeeded)
                throw new ApiException("Hibás felhasználónév vagy jelszó.", 401);
